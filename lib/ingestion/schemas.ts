...
export const EventIngestionSchema = z
  .object({
    title: z
      .string()
      .min(2, "제목은 최소 2자 이상")
    ...
    start_date: z
      ...
      .nullable()
      .optional()
      .refine((v) => !v || !isNaN(Date.parse(v)), "start_date 날짜 형식 오류"),

    end_date: z
      .string()
      .nullable()
      .optional()
      .refine((v) => !v || !isNaN(Date.parse(v)), "end_date 날짜 형식 오류"),

    status: z.enum(["upcoming", "on_sale", "ongoing", "ended"]).optional(),

    dedup_key: z.string().min(1, "dedup_key 필수"),

    source_name: z.string().min(1, "source_name 필수"),
  })
