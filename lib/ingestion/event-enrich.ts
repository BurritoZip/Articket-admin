...
import { ENRICH_SOURCE } from "./source-trust";

/**
 * enrich 가 채운 필드의 provenance 를 기존 field_sources 에 병합.
 * enrich 는 크롤보다 신뢰도가 높게(source-trust) 잡혀 있어, 이후 크롤이 "콘서트" 같은
 * 부정확한 기본값으로 이 필드를 덮지 못한다.
 */
function mergeEnrichSources(
  existing: unknown,
  filledFields: string[],
  now: string,
): Record<string, unknown> {
  const fs = { ...((existing as Record<string, unknown> | null) ?? {}) };
  for (const f of filledFields) fs[f] = { source: ENRICH_SOURCE, at: now };
  return fs;
}
...
JSON만 답해: {"ticket_open_date":"YYYY-MM-DD 또는 null","ticket_close_date":"YYYY-MM-DD 또는 null"}`;
    ...
    const patch: Record<string, unknown> = { ticket_checked_at: now };
    try {
      ...
      if (m) {
        ...
        if (open) {
          patch.ticket_open_date = `${open}T00:00:00+00:00`;
          filledFields.push("ticket_open_date");
        }
        if (close) {
          patch.ticket_close_date = `${close}T23:59:59+00:00`;
          filledFields.push("ticket_close_date");
        }
        if (filledFields.length) {
          patch.field_sources = mergeEnrichSources(
            e.field_sources,
            filledFields,
            now,
          );
...
export async function enrichEventDescriptions(
  ...
  const { data: events } = await db
    .from("events")
    .select("id,title,start_date,source_urls,booking_url,field_sources")
    ...
    if (meta) {
      await db
        ...
        .update({
          ...
          field_sources: mergeEnrichSources(
            e.field_sources,
            ["notice_text"],
            now,
          ),
...
홍보 문구·이모지·해시태그 없이 담백하게. 250자 이내.`;
    ...
    const patch: Record<string, unknown> = { description_checked_at: now };
    const raw = r.value;
    if (
      raw &&
...
export async function backfillEventPosters(
  ...
  const { data: events } = await db
    .from("events")
    .select("id,source_urls,booking_url,field_sources")
    .is("poster_url", null)
  ...
  for (const e of events ?? []) {
    ...
    const patch: Record<string, unknown> = { poster_checked_at: now };
    if (match) {
...
export async function enrichEventAges(
  ...
  for (const event of events ?? []) {
    ...
    const patch: Record<string, unknown> = { age_checked_at: now };
    if (r.value) {
