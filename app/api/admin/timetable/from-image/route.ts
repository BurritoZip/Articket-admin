...
export async function POST(request: Request) {
    ...
    return NextResponse.json(
      {
        ...
        detail: "GEMINI_API_KEY 환경 변수가 없습니다.",
      },
      { status: 500 },
    );
  }
  ...
  const dateString =
    (formData.get("date_string") as string | null)?.trim() ?? "";
  ...
  try {
    ...
    try {
      ...
      return NextResponse.json(
        {
          error: "parse_failed",
          detail: "Gemini 응답에서 JSON을 파싱하지 못했습니다.",
          raw: text,
        },
        { status: 500 },
      );
